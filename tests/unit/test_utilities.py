...
from aoc import utilities


@patch("aoc.utilities.import_utilities")
def test_load_module_locates_and_bootstraps_module(mock_importer):
    ...
    mock_importer.find_spec.assert_called_once_with(name=f"aoc.1900.day_01")
...
def test_load_module_return_none_for_unavailable_modules(mock_importer):
    ...
    mock_importer.find_spec.assert_called_once_with(name="aoc.1900.day_01")
    mock_importer.module_from_spec.assert_not_called()


@patch("aoc.utilities.os")
@patch("aoc.utilities.import_utilities")
def test_load_module_auto_creates_new_year_modules(mock_importer, mock_os):
    ...
    mock_importer.find_spec.assert_called_once_with(name="aoc.1900.day_01")
    mock_os.path.join.assert_called_once_with(ANY, "aoc", "1900")
