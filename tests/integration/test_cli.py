...
def mock_puzzle_resources(request, project_directory, tmp_path):
    ...
    original_module_location = os.path.join(project_directory, f"src/aoc/{year}")
...
def test_run_with_missing_puzzle(mock_puzzle_resources, project_directory):
    year, day = (2021, 11)
    result = os.popen(f"aoc --date {year} {day}").read()

    assert result == (
        f"Saved puzzle input to {project_directory}/src/aoc/{year}/day_{day}.txt\n"
        f"Saved puzzle input to {project_directory}/tests/unit/{year}/test_day_{day}.txt\n"
        f"Created puzzle module at {project_directory}/src/aoc/{year}/day_{day}.py\n"
...
def test_run_with_integer_answers():
    result = os.popen("aoc --date 2022 1").read()
...
def test_run_with_string_answers():
    result = os.popen("aoc --date 2022 5").read()
