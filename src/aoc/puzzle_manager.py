...
from aoc.object_types import ExamplePuzzleInputParser, PuzzleMetadata
...
def create_puzzle_resources(year: int, day: int, puzzle_input_only: bool = False) -> None:
    metadata = _build_puzzle_metadata(year=year, day=day)
    _save_puzzle_inputs(metadata=metadata)
    if not puzzle_input_only:
